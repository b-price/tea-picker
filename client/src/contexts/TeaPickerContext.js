...
export function GetData(recordID, recordField) {
  
  useEffect(() => {
    async function fetchData() {
      const id = recordID?.toString() || undefined;
      
      const response = await fetch(
        id? `http://localhost:5050/tea/${id}`
        : `http://localhost:5050/tea`
      )
      if (!response.ok) {
...
export function SetData(data, id) {
    async function setData() {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5050/tea${id ? "/"+id : ""}`, {
...
export function DeleteData(id) {
    async function deleteData() {
        if(!id) return
        await fetch(`http://localhost:5050/tea/${id}`, {
            method: "DELETE",
        })
    }
    deleteData()
